                    ...
                    if data.get("status", {}).get("completed"):
                        for node_id, output in data.get("outputs", {}).items():
                            if output.get("images"):
                                ...
                                output_file = f"brown_hair_base_{timestamp}.png"
...
def create_eye_mask(image_path):
    ...
    # Save mask
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    mask_file = f"brown_hair_eye_mask_{timestamp}.png"
    mask.save(mask_file)
                    ...
                    if data.get("status", {}).get("completed"):
                        for node_id, output in data.get("outputs", {}).items():
                            if output.get("images"):
                                ...
                                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                                output_file = f"brown_hair_FINAL_{timestamp}.png"
...
def main():
    ...
    print("\n" + "="*80)
    print("👩 BROWN-HAIRED YOUTH FITNESS INFLUENCER")
    print("="*80)
    print("\n🎯 Complete Workflow:")
    ...
    base_file, base_comfy_name = generate_base_brown_hair()
